...
export default {
  ...
  props: {
    startDate: {
      type: Date,
      default: () => new Date() // 如果没有传递startDate 取当前时间
    }
  },
  data() {
    return {
  ...
  created() {
    ...
    this.currentMonth = this.startDate.getMonth() + 1 // 大年月份
    this.yearList = Array.from(Array(11), (value, index) => index + this.currentYear - 5)
    this.dateChange() // 设置最新日历
  },
  methods: {
    dateChange() {
      // 新的日期
      this.currentDate = new Date(`${this.currentYear}-${this.currentMonth - 1}`) // 以该月1号开始时间
    },
    //  判断是否是周末
    isWeek(date) {
